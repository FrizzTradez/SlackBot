...
class Overnight_Equity(Base_Periodic):
    ...
    def process_product(self, product_name):
        try:
            ...
            if not variables:
                logger.error(f" OVERNIGHT_EQUITY | process_product | Product: {product_name} |  Note: No data available ")
                return
            ...
            if day_high < overnight_high and day_low > overnight_low:
                ...
                if channel:
                    ...
                    logger.info(f" OVERNIGHT_EQUITY | process_product | Note: Message sent to {channel} for {product_name}")
                else:
                    logger.error(f" OVERNIGHT_EQUITY | process_product | Note: No Slack channel configured for {product_name}")
            else:
                logger.info(f" OVERNIGHT_EQUITY | process_product | Product: {product_name} | Note: Overnight Stat Complete")
        except Exception as e:
            logger.error(f" OVERNIGHT_EQUITY | process_product | Product: {product_name} | Error processing: {e}")